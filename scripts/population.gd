...
func select_in_all_species(total_pop):
  ...
  # species[0].calculate_avg_fitness()
  # calculate_all_species_adj_fitness()
  species[0].parent_genomes = []
  # distribute_parents(total_pop * selection_rate)
  select_parents(round(total_pop * selection_rate))
  assert(species[0].parent_genomes.size() > 0)
  # kill_empty_species()
  # fill_parent_genomes()

func select_parents(total_parents: int):
  species[0].population_fraction = 1.0
  species[0].select_in_species(total_parents) 
